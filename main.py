...
def get_bet():
    while True:
        amount = input("What would like to Bet? $ ")
        if amount.isdigit():
            amount = int(amount)
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet >= balance:
            print(f"You not have enough amount to bet. Your Current Balance is ${balance}")
        else:
            break
    ...
    winning, winning_lines = check_winning(slots, lines, bet, symbol_values)
...
def main():
    ...
    while True:
        ...
        ans = input("Press enter to spin. (q for Quit)")
        if ans == "q":
            break
